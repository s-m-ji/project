...
// ==========================================================================================================================
	...
	function photoModal_TEST(savePath, replyJson, replyListJson, photoReviewJson, r_no, index1) {
		  ...
		  // 'photoReview'와 'replyList'가 객체 배열이라고 가정합니다.

		// 새로운 객체를 저장할 빈 배열 생성
		const newObjects = [];

		// 'photoReview' 배열 반복
		photoReview.forEach((photoItem) => {
		    // 'r_no'를 기반으로 일치하는 'replyList' 객체 찾기
		    const matchedReply = replyList.find((replyItem) => replyItem.r_no === photoItem.r_no);

		    // 일치하는 댓글이 있는지 확인
		    if (matchedReply) {
		        // 'photoItem'과 'matchedReply' 데이터를 결합한 새로운 객체 생성
		        const combinedObject = {
		            replyItem: matchedReply
		        };

		        // 새로운 결합된 객체를 'newObjects' 배열에 추가
		        newObjects.push(matchedReply);
		    }
		});

		// 이제 'newObjects' 배열에는 'photoReview'와 'replyList'에서 가져온 데이터가 포함된 객체가 있습니다.
		console.log('newObjects : ', newObjects);
		  ...
		  console.log('r_no 출력 =======================', r_no);
		  console.log('index1 출력 : ' , index1);
			  	...
			  	photoReview.forEach(function(item, index){
			  		...
			  		if(index == index1){
			  			...
			  			photoBtn +=
		  			        ...
		  			        + '<h5>'+newObjects[index].writer+'</h5>'
		  			        + '<div class="test-score2" data-max="5" data-rate="' +newObjects[index].star +'" ></div><span style="margin-left: 10px;  font-size: 13px;  color: #ab8e7d;">'+newObjects[index].replydate+'</span>'
		  			        + '<p style="font-size:18px;">'+newObjects[index].reply+'</p>'
		  			        + '</div>'
		  					+'</div>';
			  			...
			  			photoBtn +=
					    	...
					    	+ '<div class="carousel-caption d-none d-md-block" style="top: 110%; color: black;">'
		  			        + '<h5>'+newObjects[index].writer+'</h5>'
		  			        + '<div class="test-score2" data-max="5" data-rate="' +newObjects[index].star +'" ></div><span style="margin-left: 10px;  font-size: 13px;  color: #ab8e7d;">'+newObjects[index1].replydate+'</span>'
		  			        + '<p style="font-size:18px;">'+newObjects[index].reply+'</p>'
		  			        + '</div>'
					    	+    '</div>';
