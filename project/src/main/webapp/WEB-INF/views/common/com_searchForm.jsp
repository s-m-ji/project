...
<script type="text/javascript">
	...
	function chnQnaType(type, select) {
		...
		if (type == '1') {
			...
			$('#c2').append("<option value='13'  ${ param.c2 eq '13' ? 'selected' : '' } >밥/죽/떡</option>'");
			$('#c2').append("<option value='13'  ${ param.c2 eq '14' ? 'selected' : '' } >반찬</option>'");
			$('#c2').append("<option value='13'  ${ param.c2 eq '15' ? 'selected' : '' } >차/음료/술</option>'");
		} else if (type == '2') {
			...
			$('#c2').append("<option value='23'  ${ param.c2 eq '24' ? 'selected' : '' } >달걀/유제품</option>'");
			$('#c2').append("<option value='23'  ${ param.c2 eq '25' ? 'selected' : '' } >채소류</option>'");
		} else if (type == '3') {
			$('#c2').append("<option value='31'  ${ param.c2 eq '31' ? 'selected' : '' } >안주</option>'");
			$('#c2').append("<option value='32'  ${ param.c2 eq '32' ? 'selected' : '' } >도시락</option>'");
			$('#c2').append("<option value='33'  ${ param.c2 eq '33' ? 'selected' : '' } >간식</option>'");
			$('#c2').append("<option value='33'  ${ param.c2 eq '34' ? 'selected' : '' } >이유식</option>'");
			$('#c2').append("<option value='33'  ${ param.c2 eq '35' ? 'selected' : '' } >초스피드</option>'");
		}
	...
	<form action="./com_list" name="searchForm">
		...
		<div class="container text-center">
			<input type="hidden" name="pageNo" value="${pDto.cri.pageNo}">
				...
				<div class="p-2">
				<div class="recListCnt">
		      		<p style="font-size:20px">총 <b class="badge bg-danger" style="font-size:26px; transform: translateY(-3px);">${tCnt} </b> 개의 맛있는  레시피가 있습니다.</p>
		    	</div>
				</div>
				<div class="p-2">
